...
지금 상태와 지금 하는 행동이라는 조건 하에 월드에서 리워드가 발생하고, 다음 상태가 정의된다. 위 식은 `다음 상태가 S'으로 정의되고 보상을 받을 확률`이 
`현재 상태, 에이전트가 월드에 대해 한 행동에 대해, 월드가 에이전트에게 주는 리워드가 정해지고, 이로 인해 특정 상태가 결정될 확률`과 같다는 뜻이다. *솔직히 무슨 말인지 잘 모르겠는데 일단은 그렇구나 함*
